...
const Flashcard = () => {
  ...
  useEffect(() => {
    fetch(`https://localhost:7121/api/FlashcardTopic/GetFlashcardByFcTopicByUserId/` + jwtDecode(token).userID)
      .then((res) => {
        return res.json();
      })
      .then((response) => {
  ...
  return (
    <div className={cx("wrapper")}>
      <h3>My Flashcard </h3>
      <Link to={routes.createFlashcard} className={cx("btn-create")}>
        Create
      </Link>
      <div className={cx("container")}>
        <div className={cx("fcList")}>
