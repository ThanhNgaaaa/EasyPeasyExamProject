...
import styles from "./UpdateExam.module.scss";
...
const UpdateExam = () => {
  ...
  useEffect(() => {
    fetch(`https://localhost:7121/api/Exams/${ExamId}`)
  ...
  const handleFormSubmit = (e) => {
    ...
    console.log(userId);
    axios
      .put(`https://localhost:7121/api/Exams/${ExamId}`, formData)
      .then((response) => {
        console.log(response.data);
        navigate(routes.quizStudent);
      })
      .catch((error) => {
        console.log(error);
      });
      ...
      <Box className={cx("container")}>
        ...
        <form onSubmit={handleFormSubmit}>
          ...
          <div className={cx("actions")}>
            <Link to={routes.quizStudent} className={cx("btn-exit")}>
              EXIT
            </Link>
